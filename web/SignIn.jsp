    ...
    <body>
        ...
        FB.login(function(response) {
          if (response.status === 'connected') {
            // Lấy access token và chuyển hướng tới server
            const accessToken = response.authResponse.accessToken;
            window.location.href = `facebook-callback.jsp?accessToken=${accessToken}`;
          } else {
            alert('Đăng nhập thất bại hoặc bị hủy.');
          }
        }, {scope: 'public_profile,email'}); // Yêu cầu các quyền
      }
        </script>
        ...
        <script>
          window.fbAsyncInit = function() {
            FB.init({
              ...
              cookie     : true,
              xfbml      : true,
              version    : 'v16.0' // Phiên bản API Facebook
            });
